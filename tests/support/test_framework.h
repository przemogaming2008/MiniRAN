...
inline void writeJUnit(const std::vector<TestResult>& results,
                       const std::string& path)
{
    ...
    output << "<testsuite name=\"MiniRAN\" tests=\"" << results.size()
           << "\" failures=\"" << failures
           << "\" errors=\"0\" time=\""
    ...
    for (const auto& result : results) {
        output << "  <testcase name=\"" << escapeXml(result.name)
                       ...
                       int line)
{
    if (!condition) {
        std::ostringstream output;
        output << file << ':' << line
               << " ASSERT_TRUE failed: " << expression;
        throw TestFailure(output.str());
    }
}

inline void assertTrueMessage(bool condition,
                              ...
                              int line)
{
    if (!condition) {
        std::ostringstream output;
        output << file << ':' << line
               << " ASSERT_TRUE failed: " << message;
        throw TestFailure(output.str());
    }
}

template <typename Left, typename Right>
void assertEqual(const Left& left,
                 const Right& right,
                 const char* leftExpr,
                 ...
                 int line)
{
    if (!(left == right)) {
        ...
        output << file << ':' << line
               ...
               << leftExpr << " != " << rightExpr
               << " (" << toDebugString(left)
               << " vs " << toDebugString(right) << ')';
        throw TestFailure(output.str());
    }
}

template <typename Left, typename Right>
void assertNotEqual(const Left& left,
                    const Right& right,
                    const char* leftExpr,
                    const char* rightExpr,
                    const char* file,
...
inline void assertNear(double left,
                       double right,
                       double tolerance,
                       const char* leftExpr,
                       const char* rightExpr,
                       ...
                       int line)
{
        ...
        tolerance < 0.0)
    {
        std::ostringstream output;
        output << file << ':' << line
               << " ASSERT_NEAR failed: non-finite value or negative tolerance. "
               << leftExpr << '=' << std::setprecision(10) << left
               << ", "
               << rightExpr << '=' << std::setprecision(10) << right
               << ", "
               << toleranceExpr << '=' << std::setprecision(10) << tolerance;
        throw TestFailure(output.str());
    }
...
#define ASSERT_TRUE(expression) \
    ::miniran::test::assertTrue((expression), #expression, __FILE__, __LINE__)

#define ASSERT_EQ(left, right) \
    ::miniran::test::assertEqual((left), (right), #left, #right, __FILE__, __LINE__)

#define ASSERT_NE(left, right) \
    ::miniran::test::assertNotEqual((left), (right), #left, #right, __FILE__, __LINE__)

#define ASSERT_NEAR(left, right, tolerance)                                \
    ::miniran::test::assertNear(                                            \
